...
import { Upload, FileImage, X, Download, AlertCircle, ChevronRight, CheckCircle2, Loader2 } from "lucide-react"
...
export default function HEICToJPGClient() {
  ...
  const downloadAll = async () => {
    ...
    const zip = new JSZip()
    completed.forEach(f => {
      const name = f.file.name.replace(/\.(heic|heif)$/i, '.jpg')
      zip.file(name, f.resultBlob!)
    })

    const content = await zip.generateAsync({ type: "blob" })
    saveAs(content, "toolhive-converted-heic.zip")
  }
    ...
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      <div className="mb-12 text-center max-w-2xl mx-auto">
        <h1 className="font-heading text-4xl sm:text-5xl font-extrabold mb-4">
          HEIC to <span className="text-brand-orange">JPG</span> Converter
        </h1>
        <p className="text-muted-foreground text-lg">
          Fast, private, and 100% free. Convert your iPhone photos to universally supported JPG format without uploading them.
        </p>
      </div>
      ...
      <div className="space-y-8">
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-2xl font-bold font-syne mb-2">Drop HEIC photos here</h2>
            <p className="text-muted-foreground mb-6">or click to browse from your device</p>
            <Button size="lg" className="bg-brand-orange hover:bg-brand-orange/90 text-white rounded-full px-8 h-12 text-lg font-bold">
              Select Photos
            </Button>
          </div>
        </div>
                    ...
                    <div className="flex items-center gap-4">
                      {item.status === 'processing' && (
                        <div className="flex items-center gap-2 text-brand-orange">
                          ...
                          <span className="text-xs font-bold uppercase tracking-wider">Converting</span>
                        </div>
                      )}
